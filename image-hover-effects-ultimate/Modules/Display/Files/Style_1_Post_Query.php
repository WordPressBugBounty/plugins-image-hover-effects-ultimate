...
class Style_1_Post_Query {
    ...
    public function post_query($rawdata, $args, $style) {
        ...
        if (!is_array($styledata)):
            ?><p> Style Data not found. Kindly Check Display Post <a href="https://wpkindemos.com/imagehover/docs/hover-extension/display-post/">Documentation</a>.</p><?php
            return;
        endif;

        $child = $this->wpdb->get_row($this->wpdb->prepare("SELECT * FROM $this->child_table WHERE styleid = %d", $styleid), ARRAY_A);
        if (!is_array($child)):
            ?><p>Set Initial Data How to Decorate your Display Post. Kindly Check Display Post <a href="https://wpkindemos.com/imagehover/docs/hover-extension/display-post/">Documentation</a>.</p><?php
            return;
        endif;
